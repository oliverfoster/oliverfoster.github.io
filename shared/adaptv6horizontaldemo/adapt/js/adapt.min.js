...
define('core/js/adapt',[
  ...
  var Adapt = Backbone.Model.extend({
    ...
    initialize: function () {
      this.setupWait();
      this.setupDeprecatedComponentStore();
    },

    setupDeprecatedComponentStore: function() {
      Object.defineProperty(this, 'componentStore', {
        get: function() {
          Adapt.log && Adapt.log.warn('Adapt.componentStore is deprecated, please use Adapt.store instead');
          return this.store;
        }
      });
    },
    ...
    navigateToElement: function(selector, settings) {
      ...
      // Get current contentobject to check whether this is the current contentobject
      var currentContentObject = (currentModel.isContentObject()) ? currentModel : currentModel.findAncestor('contentObjects');

      // If current page - scrollTo element
      if (currentContentObject.get('_id') === this.location._currentId) {
      ...
      this.once('contentObjectView:ready', _.debounce(function() {
      ...
      Backbone.history.navigate('#/id/' + currentContentObject.get('_id'), {trigger: true, replace: shouldReplaceRoute});
    },
    ...
    register: function(name, object) {
      if (!object.view && !object.model || object instanceof Backbone.View) {
        Adapt.log.warn('View-only registrations are deprecated');
        object = { view: object };
      }
      ...
      this.store[name] = _.extend({}, this.store[name], object);
     ...
     * @param {string|Backbone.Model|object} nameOrModelOrData The name of the view class you want to fetch e.g. `"hotgraphic"` or its model or its json data
     */
    getViewName: function(nameOrModelOrData) {
      if (typeof nameOrModelOrData === "string") {
        return nameOrModelOrData;
      }
      if (nameOrModelOrData instanceof Backbone.Model) {
        nameOrModelOrData = nameOrModelOrData.toJSON();
      }
      if (nameOrModelOrData instanceof Object) {
        return nameOrModelOrData._view || nameOrModelOrData._component || nameOrModelOrData._type;
      }
     ...
     */
    getViewClass: function(nameOrModelOrData) {
      var name = this.getViewName(nameOrModelOrData);
     ...
     * @param {string|object} name The name of the model you want to fetch e.g. `"hotgraphic"` or its json data
     */
    getModelName: function(nameOrData) {
      if (typeof nameOrData === "string") {
        return nameOrData;
      }
      if (nameOrData instanceof Object) {
        return nameOrData._model || nameOrData._component || nameOrData._type;
      }
     ...
     */
    getModelClass: function(nameOrData) {
      var name = this.getModelName(nameOrData);
      var object = this.store[name];
      if (!object) {
          throw Error('The component "' + name + '" doesn\'t exist in your project');
      }
     ...
     */
    mapById: function(id) {
      this.log && this.log.deprecated('Adapt.mapById, is no longer used internally.');
      var model = this.findById(id);
      if (!model) return;
      var type = model.get("_type") || '';
      if (type === 'course' || type.slice(-1) === 's') return type;
      return type+'s';
    },
    ...
    parseRelativeString: function(relativeString) {
      ...
      // TODO: hard coded type references need removing
...
define('core/js/logging',[
  ...
  var Logging = Backbone.Controller.extend({
    ...
    removed: function() {
      ...
      this.warn.apply(Adapt.log, ['REMOVED:'].concat(args));
      return this;
    },
    ...
    deprecated: function() {
      ...
      this.warn.apply(Adapt.log, ['DEPRECATED:'].concat(args));
      return this;
    },
    ...
    _hasWarned: function(args) {
      if (this._config._level === LOG_LEVEL.DEBUG) {
        return false;
      }
      var hash = _.map(args, String).join(':');
      if (this._warned[hash]) {
    ...
    _canWarn: function() {
      return this._config._level <= LOG_LEVEL.INFO;
    },
...
define('core/js/models/adaptModel',[
  ...
  var AdaptModel = Backbone.Model.extend({
    ...
    isStructuralContentObject: function() {
      return Boolean(this.constructor._isStructuralContentObject);
    },

    isContentObject: function() {
      return Boolean(this.constructor._isContentObject);
    },

    isChildless: function() {
      return Boolean(this.constructor._isChildless);
    },

    isNavigationRoot: function() {
      return Boolean(this.constructor._isNavigationRoot);
    },

    setupModel: function() {
      var hasChildren = this.getChildren().length !== 0;
      if (hasChildren) {
    ...
    isType: function(type) {
      var types = _.filter([
        type,
        (type.slice(-1) !== 's') ? null : type.slice(0, -1) // depluralize if ending in s
      ], Boolean);
      if (_.includes(types, 'contentObject') && this.isContentObject) {
        return true;
      }
      if (_.includes(types, this.get("_type"))) {
        return true;
      }
      return false;
    },
    ...
    findAncestor: function (ancestorType) {
      ...
      if (!ancestorType || parent.isType(ancestorType)) {
        return parent;
      }
    ...
    findDescendantModels: function(descendants, options) {
      ...
      var returnedDescendants = allDescendantsModels.filter(function(model) {
        return model.isType(descendants);
      });
    ...
    findRelativeModel: function(relativeString, options) {
      ...
      // TODO: rewrite without hardcoded types
      ...
      var rootModel = Adapt.data.getNavigationRootModel();
    ...
    getChildren: function () {
      ...
      var children = Adapt.data.where({ _parentId: this.get("_id") });
    ...
    getSiblings: function (passSiblingsAndIncludeSelf) {
      ...
      if (!passSiblingsAndIncludeSelf) {
        ...
        siblings = _.reject(Adapt.data.where({
    ...
    setOnChildren: function (key, value, options) {
      ...
      for (var i = 0, len = models.length; i < len; i++) {
        var child = models[i];
        child.setOnChildren && child.setOnChildren.apply(child, args);
      }
    ...
    _isStructuralContentObject: false,
    _isContentObject: false,
    _isChildless: false,
    _isNavigationRoot: false
  });
...
define('core/js/models/courseModel',[
  ...
  var CourseModel = ContentObjectModel.extend({

    initialize: function(attrs, options) {
      ContentObjectModel.prototype.initialize.apply(this, arguments);
    ...
    _isStructuralContentObject: true,
    _isContentObject: true,
    _isChildless: false,
...
define('core/js/data',[
  ...
  var Data = AdaptCollection.extend({

    model: function(json) {
      var modelName = Adapt.getModelName(json);
      var viewName = Adapt.getViewName(json);
      if (!modelName && !viewName) {
        // if no model or view registered use a backbone model
        return new Backbone.Model(json);
      }
      var ModelClass = Adapt.getModelClass(modelName);
      if (ModelClass) {
        // use registered model
        return new ModelClass(json);
      }
      var ViewClass = Adapt.getViewClass(viewName);
      if (ViewClass && ViewClass._isQuestionType) {
        // use question model
        Adapt.log.deprecated('QuestionModel inference, please register a model with your QuestionView');
        return Adapt.getModelClass('question');
      }
      // use component model
      Adapt.log.deprecated('ComponentModel inference, please register a model with your ComponentView');
      return Adapt.getModelClass('component');
    },
    ...
    loadCourseData: function(callback, newLanguage) {
      ...
      Adapt.wait.queue(function() {
        this.dataIsLoaded(callback, newLanguage);
      }.bind(this));
      ...
      this.loadLanguageDataManifest(courseFolder)
        .then(function() {
          this.trigger('loaded');
        }.bind(this));
    ...
    loadLanguageDataManifest: function(languagePath) {
      this.languagePath = languagePath;
      return this.getManifest()
        .then(this.getManifestDataFiles.bind(this))
        .then(this.addManifestDataFiles.bind(this))
        .then(function languageDataAdded() {
          this.trigger("reset");
        }.bind(this));
    },

    getManifest: function() {
      var manifestPath = this.languagePath + '_language_data_manifest.js';
      return this.getJSON(manifestPath)
    },

    getManifestDataFiles: function(languageDataManifest) {
      return Promise.all(languageDataManifest.map(function(filePath) {
        return this.getJSON(this.languagePath+filePath);
      }.bind(this)));
    },

    addManifestDataFiles: function(allFileDatas) {
      allFileDatas.forEach(function(fileData) {
        var fileDataAsArray = (fileData instanceof Array) ? fileData : [fileData];
        fileDataAsArray.forEach(function(modelAttributes) {
          this.push(modelAttributes);
        }.bind(this));
      }.bind(this));
    },

    dataIsLoaded: function(callback, newLanguage) {
    ...
    getNavigationRootModel: function() {
      return this.find(function(model) {
        // find the first model which isn't the course marked as the navigation root
        return model.isNavigationRoot() && !(model instanceof CourseModel);
      }) || this.find(function(model) {
        // or find the first model marked as the navigation root, including the course
        return model.isNavigationRoot();
      });
    }
...
define('core/js/collections/adaptSubsetCollection',[
  ...
  var AdaptSubsetCollection = AdaptCollection.extend({

    initialize : function(models, options) {
      this.parent = options.parent;
      this.listenToOnce(this.parent, 'reset', this.loadSubset);
    },

    loadSubset: function() {
      var subset = this.parent.filter(function(model) {
        if (model instanceof this.model) return true;
      }.bind(this));
      this.set(subset);
    }
...
define('core/js/models/menuModel',[
  ...
  var MenuModel = ContentObjectModel.extend({},{
    _isStructuralContentObject: true,
    _isContentObject: true,
    _isChildless: false,
    _isNavigationRoot: false
...
define('core/js/models/pageModel',[
  ...
  var PageModel = ContentObjectModel.extend({}, {
    _isStructuralContentObject: false,
    _isContentObject: true,
    _isChildless: false,
    _isNavigationRoot: false
...
define('core/js/models/blockModel',[
  ...
  var BlockModel = AdaptModel.extend({
    ...
    getChildren: function () {
      ...
      if (childrenCollection.length == 2 &&
...
define('core/js/views/adaptView',[
  ...
  var AdaptView = Backbone.View.extend({
    ...
    render: function() {
      ...
      if (this.model.isContentObject()) {
        Adapt.trigger('contentObjectView:preRender', this);
      }
      Adapt.trigger('view:preRender', this);
      ...
      if (this.model.isContentObject()) {
        Adapt.trigger('contentObjectView:render', this);
      }
      ...
      _.defer(function() {
        ...
        if (this.model.isContentObject()) {
          Adapt.trigger('contentObjectView:postRender', this);
        }
        Adapt.trigger('view:postRender', this);
      }.bind(this));
...
], function(Adapt, AdaptView, BlockView) {
  ...
  var PageView = AdaptView.extend({
    ...
    isReady: function() {
      ...
      var performIsReady = function() {
        ...
        Adapt.trigger('contentObjectView:ready', this);
...
], function(Adapt, Data, AdaptSubsetCollection, ContentObjectModel, MenuModel, PageModel, ArticleModel, BlockModel, ComponentModel, QuestionModel, PageView) {

  var MPABC = Backbone.Controller.extend({

    initialize: function() {
      this.listenTo(Adapt.data, {
        'loading': this.waitForLoaded,
        'loaded': this.checkDataIsLoaded
      });
      this.setupSubsetCollections();
      this.setupDeprecatedMapAdaptIdsToObjects();
    },
    ...
    checkDataIsLoaded: function() {
      var isDataLoaded = Boolean(Adapt.course);
      if (!isDataLoaded) {
        return;
      }
      Adapt.wait.end();
    ...
    setupSubsetCollections: function() {
      Adapt.contentObjects = new AdaptSubsetCollection(null, { parent: Adapt.data, model: ContentObjectModel });
      Adapt.articles = new AdaptSubsetCollection(null, { parent: Adapt.data, model: ArticleModel });
      Adapt.blocks = new AdaptSubsetCollection(null, { parent: Adapt.data, model: BlockModel });
      Adapt.components = new AdaptSubsetCollection(null, { parent: Adapt.data, model: ComponentModel });
    },

    setupDeprecatedMapAdaptIdsToObjects: function () {
      Object.defineProperty(Adapt.contentObjects, '_byAdaptID', {
        get: function() {
          Adapt.log.deprecated('Adapt.contentObject._byAdaptId, please use Adapt.data,_byAdaptId instead');
          return Adapt.contentObjects.groupBy('_id');
        }
      });
      Object.defineProperty(Adapt.articles, '_byAdaptID', {
        get: function() {
          Adapt.log.deprecated('Adapt.articles._byAdaptId, please use Adapt.data,_byAdaptId instead');
          return Adapt.articles.groupBy('_id');
        }
      });
      Object.defineProperty(Adapt.blocks, '_byAdaptID', {
        get: function() {
          Adapt.log.deprecated('Adapt.blocks._byAdaptId, please use Adapt.data,_byAdaptId instead');
          return Adapt.blocks.groupBy('_id');
        }
      });
      Object.defineProperty(Adapt.components, '_byAdaptID', {
        get: function() {
          Adapt.log.deprecated('Adapt.components._byAdaptId, please use Adapt.data,_byAdaptId instead');
          return Adapt.components.groupBy('_id');
        }
      });
    }
...
define('core/js/router',[
  ...
  var Router = Backbone.Router.extend({
    ...
    handleRoute: function() {
      ...
      if (Adapt.router.get('_canNavigate')) {
        ...
        switch (args.length) {
          ...
          case 1:
          default:
            // If only one parameter assume it's the ID or if none, assume it's the course home page.
    ...
    handleId: function(id) {
      var rootModel = Adapt.data.getNavigationRootModel();
      var currentModel = (!id) ? rootModel : Adapt.findById(id);
      ...
      if (currentModel.isNavigationRoot() && Adapt.course.has('_start')) {
      ...
      if (currentModel.isContentObject() && currentModel.get('_isLocked') && Adapt.config.get('_forceRouteLocking')) {
        // Locked id
      ...
      if (currentModel.isContentObject()) {
        // Move to a content object
        this.showLoading();
        this.removeViews(_.bind(function() {
          this.setLocation(currentModel, _.bind(function() {
            Adapt.once('contentObjectView:ready', function() {
              // Allow navigation.
              Adapt.router.set('_canNavigate', true, { pluginName: 'adapt' });
              this.handleNavigationFocus();
            }.bind(this));
            var type = currentModel.get('_type');
            Adapt.trigger('router:'+type, currentModel);
            if (currentModel.isNavigationRoot() && type === 'course') {
              // TODO: remove this later when menus have registered their views
              Adapt.trigger('router:menu', currentModel);
            }
            Adapt.trigger('router:contentObject', currentModel);
            var ViewClass = Adapt.getViewClass(currentModel);
            if (!ViewClass) return;
            this.$wrapper.append(new ViewClass({ model: currentModel }).$el);
          }, this));
        }, this));
        return;
      }
    ...
    navigateToPreviousRoute: function(force) {
      ...
      if (Adapt.router.get('_canNavigate') || force) {
        var current = Adapt.location.current;
        var previous = Adapt.location.previous;
        if (!current) {
          // if there is no current, go back in browser history
          return Backbone.history.history.back();
        }
        var hasPreviousNonStructural = (previous && !previous.isStructuralContentObject());
        if (hasPreviousNonStructural && current.isStructuralContentObject()) {
          // if previous was a non-structural content object (page)
          // and current is a structral content object (menu)
          // go to parent
          return this.navigateToParent();
        }
        if (hasPreviousNonStructural) {
          // if has a previous location which is non-structural (page)
          // go back in the browser history
          return Backbone.history.history.back();
        }
        if (current.isNavigationRoot()) {
          // if at navigation root (course) then back button is inactive
          return;
        }
        // if no previous, drop to parent
    ...
    navigateToCurrentRoute: function(force) {
      if (Adapt.router.get('_canNavigate') || force) {
        ...
        var rootModel = Adapt.data.getNavigationRootModel();
        var route = (currentId === rootModel.get('_id')) ? '#/' : '#/id/' + currentId;
    ...
    navigateToParent: function(force) {
      if (Adapt.router.get('_canNavigate') || force) {
        ...
        var rootModel = Adapt.data.getNavigationRootModel();
        var route = (parentId === rootModel.get('_id')) ? '#/' : '#/id/' + parentId;
    ...
    setLocation: function(currentModel, onComplete) {
      this.setContentObjectToVisited(currentModel);
      var type = currentModel.get('_type');
      var id = currentModel.get('_id');
      if (currentModel.isNavigationRoot() && currentModel.get('_type') === 'course') {
        this.updateLocation('course', type, id, onComplete);
        return;
      }
      var location = type + '-' + id;
      this.updateLocation(location, type, id, onComplete);
    },

    updateLocation: function(currentLocation, type, id, onComplete) {

      Adapt.location.previous = Adapt.location.current;
      Adapt.location.current = Adapt.findById(id);
      ...
      if (currentLocation === 'course') {
        Adapt.location._currentId = Adapt.location.current.get('_id');
        ...
        Adapt.location._lastVisitedType = type;

        // TODO: remove this if possible, should have no hardcoded types
        if (type === 'menu') {
    ...
    setDocumentTitle: function() {
      ...
      if (currentModel && !currentModel.isNavigationRoot()) {
...
define('core/js/views/navigationView',[
  ...
  var NavigationView = Backbone.View.extend({
    ...
    initialize: function() {
      ...
      this.listenTo(Adapt, 'router:contentObject', this.hideNavigationButton);
      this.template = "nav";
      this.preRender();
    },
...
define('components/adapt-horizontalRootPage/js/HorizontalBlocksRootPageView',[
  'core/js/adapt',
  'core/js/views/pageView'
], function(Adapt, PageView) {

  var HorizontalBlocksRootPageView = PageView.extend({
    ...
    initialize: function() {
      _.bindAll(this, 'draw');
      this.listenTo(Adapt, {
        'contentObjectView:ready': this.onReady,
        'device:resize': this.draw,
        'page:scrollTo': this.onScrollTo,
    ...
    onReady: function(view) {
      if (view !== this) return;
      this.setupBlocks();
      Adapt.set("_canScroll", false, { pluginName: 'hbrpv '});
    },

    setupBlocks: function() {
      this.blockViews = _.filter(this.findDescendantViews(), function(view) {
        return view.model.get('_type') === 'block';
      });
      this.$('.block').on('resize', this.draw);
      this.currentBlock = -1;
      this.next();
    },
    ...
    onInteractionComplete: function(model, val) {
      if (!this.blockViews) return;
      var currentBlockView = this.blockViews[this.currentBlock];
      var currentBlockModel = currentBlockView.model;
      if (model.get('_id') !== currentBlockModel.get('_id')) return;
      this.draw();
    },
    ...
    draw: function() {
      ...
      var isPreviousComplete = false;
      this.blockViews.forEach(function(blockView, index) {
        var blockModel = blockView.model;
        var isIncompleteNotCurrentBlock = (!blockModel.get('_isInteractionComplete') && index !== this.currentBlock);
        if (isIncompleteNotCurrentBlock && !isPreviousComplete) {
          shouldBeVisible = false;
        }
        blockModel.setOnChildren('_isVisible', shouldBeVisible, { pluginName: 'hbrpv' });
        blockView.$el.css("display", (this.currentBlock === index) ? "" : "none");
        isPreviousComplete = blockModel.get('_isInteractionComplete');
      }.bind(this));

      var currentBlockView = this.blockViews[this.currentBlock];
      var currentBlockModel = currentBlockView.model;

      var isScrolling = (currentBlockView.$el.height() >= $(window).height() - $('.nav').height());
      this.$('.horizontalblocksrootpage__child__container__inner').toggleClass("is-scrolling", isScrolling);

      var isAtStart = (this.currentBlock <= 0);
      var isAtEnd = (this.currentBlock >= this.blockViews.length-1 || !currentBlockModel.get('_isInteractionComplete'));

      this.$('.js-back').attr('disabled', 'disabled' ).toggleClass('is-disabled', isAtStart);
      if (!isAtStart) this.$('.js-back').removeAttr('disabled');

      this.$('.js-next').attr('disabled', 'disabled' ).toggleClass('is-disabled', isAtEnd);
      if (!isAtEnd) this.$('.js-next').removeAttr('disabled');
    ...
    onScrollTo: function(selector) {
      var id = selector.slice(1);
      var model = Adapt.findById(id);
      if (!model) return;
      var parentBlock = model.findAncestor('blocks');
      var canScrollTo = false;
      var scrollToIndex = -1;
      _.find(this.blockViews, function(blockView, index) {
        var blockModel = blockView.model;
        var isDestinationBlock = (blockModel.get('_id') === parentBlock.get('_id'));
        if (!blockModel.get('_isInteractionComplete') && !isDestinationBlock) return true;
        if (!isDestinationBlock) return;
        canScrollTo = true;
        scrollToIndex = index;
        return true;
      }.bind(this));
      if (!canScrollTo) return;
      this.currentBlock = scrollToIndex;
      this.draw();
    },

    onRemove: function() {
      Adapt.set("_canScroll", true, { pluginName: 'hbrpv '});
      this.$('.block').off('resize', this.draw);
    }

  },{
    template: 'horizontalBlocksRootPage',
    childContainer: '.horizontalblocksrootpage__child__container__inner',
    type: 'page'
  });

  return HorizontalBlocksRootPageView;

});

define('components/adapt-horizontalRootPage/js/HorizontalBlocksRootPageModel',[
  'core/js/models/pageModel'
], function(PageModel) {

  var HorizontalBlocksRootPageModel = PageModel.extend({

    defaults: PageModel.resultExtend('defaults', {
      _isNavigationRoot: true
    }),

    isNavigationRoot: function() {
      return Boolean(this.get("_isNavigationRoot"));
    }

  }, {});

  return HorizontalBlocksRootPageModel;

});

define('components/adapt-horizontalRootPage/js/adapt-horizontalBlocksRootPage',[
  'core/js/adapt',
  './HorizontalBlocksRootPageView',
  './HorizontalBlocksRootPageModel'
], function(Adapt, HorizontalBlocksRootPageView, HorizontalBlocksRootPageModel) {

  return Adapt.register('horizontalBlocksRootPage', {
    model: HorizontalBlocksRootPageModel,
    view: HorizontalBlocksRootPageView
  });
...
define('core/js/views/menuView',[
  ...
  Adapt.register('menu', { view: MenuView });
...
;
define("plugins", ["components/adapt-contrib-accordion/js/adapt-contrib-accordion","components/adapt-contrib-assessmentResults/js/adapt-contrib-assessmentResults","components/adapt-contrib-blank/js/adapt-contrib-blank","components/adapt-contrib-gmcq/js/adapt-contrib-gmcq","components/adapt-contrib-graphic/js/adapt-contrib-graphic","components/adapt-contrib-hotgraphic/js/adapt-contrib-hotgraphic","components/adapt-contrib-matching/js/adapt-contrib-matching","components/adapt-contrib-mcq/js/adapt-contrib-mcq","components/adapt-contrib-media/js/adapt-contrib-media","components/adapt-contrib-narrative/js/adapt-contrib-narrative","components/adapt-contrib-slider/js/adapt-contrib-slider","components/adapt-contrib-text/js/adapt-contrib-text","components/adapt-contrib-textInput/js/adapt-contrib-textInput","components/adapt-horizontalRootPage/js/adapt-horizontalBlocksRootPage","extensions/adapt-contrib-assessment/js/adapt-assessmentArticleExtension","extensions/adapt-contrib-bookmarking/js/adapt-contrib-bookmarking","extensions/adapt-contrib-languagePicker/js/adapt-languagePicker","extensions/adapt-contrib-pageLevelProgress/js/adapt-contrib-pageLevelProgress","extensions/adapt-contrib-resources/js/adapt-contrib-resources","extensions/adapt-contrib-spoor/js/adapt-contrib-spoor","extensions/adapt-contrib-trickle/js/adapt-contrib-trickle","extensions/adapt-contrib-tutor/js/adapt-contrib-tutor","menu/adapt-contrib-boxMenu/js/adapt-contrib-boxMenu","theme/adapt-contrib-vanilla/js/theme"], function(){});
